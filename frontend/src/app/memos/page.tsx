    ...
    <div className="flex flex-1 flex-col gap-4 lg:min-h-0 lg:flex-row lg:gap-6">
      <div className="flex w-full shrink-0 flex-col gap-4 lg:min-h-0 lg:w-80">
        <div className="card flex flex-col gap-4 lg:min-h-0">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-xl font-display font-bold text-slate-900 sm:text-2xl lg:text-xl">メモ</h1>
              <p className="mt-1 text-sm text-slate-500">検索・整理しながらすばやく編集</p>
            </div>
            <button
          ...
          <div className="space-y-2 lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:pr-1">
            {isLoading ? (
              ...
              <>
                {pinned.length > 0 && (
                  <>
                    <p className="flex items-center gap-1 px-1 text-xs font-medium text-slate-400">
                      <Pin size={10} /> ピン留め
                    </p>
                    {pinned.map((m) => (
                      <MemoCard
                    ...
                    {others.length > 0 && <p className="px-1 pt-2 text-xs font-medium text-slate-400">すべて</p>}
                  </>
                )}
                {others.map((m) => (
                  <MemoCard
      ...
      <div className="min-w-0 flex-1 lg:min-h-0">
        {(creating || selected) ? (
          ...
          <div className="card flex min-h-[280px] items-center justify-center text-slate-400 lg:h-full">
